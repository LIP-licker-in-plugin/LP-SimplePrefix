package com.licker2689.lsp.functions;
...
public class LSPFunction {
    ...
    public static void showPrefixList(Player p) {
        ...
        for (String key : list) {
            ...
            if (s != null) {
                ...
                item.setItemMeta(im);
                prefixs.add(NBT.setStringTag(item, "lsp.prefix", key));
            }
        }
        LInventory inv = new LInventory(null, "§6보유 칭호 목록", 54, true, plugin);
    ...
    public static void getPrefixCoupon(Player p, String name) {
        if (plugin.config.getConfigurationSection("Settings.PrefixList").getKeys(false).contains(name)) {
            ...
            im.setDisplayName(ColorUtils.applyColor(plugin.config.getString("Settings.couponCustomName").replace("%lsp.prefix%", prefix)));
            List<String> lore = plugin.config.getStringList("Settings.couponLores");
            for (int i = 0; i < lore.size(); i++) {
                lore.set(i, ColorUtils.applyColor(lore.get(i).replace("%lsp.prefix%", prefix)));
            }
            im.setLore(lore);
            item.setItemMeta(im);
            p.getInventory().addItem(NBT.setStringTag(item, "lsp.prefix", name));
